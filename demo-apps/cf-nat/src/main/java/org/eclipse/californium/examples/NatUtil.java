...
public class NatUtil implements Runnable {
	...
	/**
	 * Get outgoing port for incoming inet address.
	 * 
	 * @param incoming inet address to remove mapping
	 * @return outgoing port. {@code -1}, if not mapping available.
	 */
	public int getPortForLocalAddress(InetSocketAddress incoming) {
		NatEntry entry = nats.get(incoming);
		if (null != entry) {
